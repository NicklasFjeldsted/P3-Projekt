...
namespace TEC_KasinoAPI.Games
{
    ...
    public class Blackjack : IBlackjack
    {
		...
		private readonly ConcurrentDictionary<string, BlackjackPlayerData> _players = new ConcurrentDictionary<string, BlackjackPlayerData>();
		...
		private void SetTurnOrder()
		{
			...
			{
				if (_gameManager.Bets[pair.Key] <= 49) continue;
		...
		private void CalculateWinners()
		{
			foreach (var pair in Players)
			{
				...
				BlackjackPlayerData oldData = new BlackjackPlayerData(pair.Value);
		...
		private void DealCards()
		{
			using (var scope = _scopeFactory.CreateScope())
			{
				...
				{
					if (_gameManager.Bets[data.Key] <= 49) continue;
				...
				foreach (var data in Players.ToList())
				{
					...
					BalanceRequest request = new BalanceRequest(data.Value.CustomerID, _gameManager.Bets.First(x => x.Key == id).Value);
		...
		{
			foreach (var player in _gameManager.Bets)
			{
		...
		public async Task EndGame(bool houseBust = false)
		{
			...
			foreach (var player in Players.Values)
			{
				...
				BalanceRequest request = new BalanceRequest(player.CustomerID, _gameManager.Bets.First(x => x.Key == connectionID).Value);
