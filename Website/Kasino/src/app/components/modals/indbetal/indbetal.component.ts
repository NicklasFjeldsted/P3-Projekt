...
export class IndbetalComponent implements OnInit {
  ...
  constructor(private dialogRef: DialogRef, @Inject(DIALOG_DATA) public data: string, private dialogService: DialogService, private builder: FormBuilder, private balance: BalanceService) { }
  ...
  onSubmit() {
    this.submitted = true;
    let formatCard = this.f['cardNumber']?.value.split('-').join('');
    console.log(this.isValid(formatCard));
    if(!this.isValid(formatCard) || this.form.invalid) {
      return;
    }
